...
namespace PlaceholderSoftware.WetStuff.Windows.Update
{
    ...
    public class UpdateLauncher
    {
        ...
        private const string CheckForNewVersionKey = "placeholder_wetstuff_update_checkforlatestversion";

        private static readonly Log Log = Logs.Create(LogCategory.Core, typeof(UpdateLauncher).Name);
        //private const string ForceCheckUpdateMenuItemPath = "Window/Wet Stuff/Check For Updates";

        private static readonly string StateKey = "placeholder_wetstuff_update_checkstate";
        ...
        private static IEnumerable<bool> CheckUpdateVersion()
        {
            ...
            yield return true;
            var state = GetUpdateState();
            Log.Trace("Beginning version check. Current state:'{0}' Current version:'{1}'", state, PlaceholderSoftware.WetStuff.WetStuff.Version);
            ...
            var request = UnityWebRequest.Get(string.Format("https://placeholder-software.co.uk/wetstuff/releases/latest-published.html{0}", EditorMetadata.GetQueryString("update_checker")));
            request.SendWebRequest();

            // Wait until request is complete
            while (!request.isDone && !request.isNetworkError) yield return true;
            ...
            else
            {
                ...
                if (latest.CompareTo(PlaceholderSoftware.WetStuff.WetStuff.Version) <= 0)
                {
                    success(state.ShownForVersion);
                    Log.Trace("Update success, newer version '{0}' already installed", PlaceholderSoftware.WetStuff.WetStuff.Version);
                    yield return false;
                }

                // Update the state so that the window does not show up again for this version
                success(latest);
                UpdateWindow.Show(latest, PlaceholderSoftware.WetStuff.WetStuff.Version);
        ...
        private static void Update()
        {
            ...
            if (_updateCheckInProgress != null)
            {
                ...
                try
                {
                    if (!_updateCheckInProgress.MoveNext() || !_updateCheckInProgress.Current) _updateCheckInProgress = null;
                }
                catch (Exception e)
                {
            ...
            if (_frameCount % 10000 != 250) return;
        ...
        internal static bool GetUpdaterEnabled()
        {
            ...
            if (!EditorPrefs.HasKey(CheckForNewVersionKey)) return true;
