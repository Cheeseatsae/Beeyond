...
fixed4 mask_frag(v2f i) : SV_Target
{
    ...
    wetness = yMaskWetness * pow(saturate(dot(normalize(i.orientation), wnormal)), _EdgeSharpness);
    ...
    weights = pow(weights, _EdgeSharpness);
    weights = weights / dot(weights, 1);

    wetness = dot(maskWetness, weights);
#else
    wetness = pow(saturate(dot(normalize(i.orientation), wnormal)), _EdgeSharpness);
#endif
